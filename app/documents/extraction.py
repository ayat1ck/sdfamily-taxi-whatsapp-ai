...
Если на одном фото несколько документов — выбери главный document_type и добавь остальные типы в additional_document_types.
...
- Для vehicle_registration_doc (СТС / техпаспорт) НЕ заполняй full_name, iin, birth_date —
  там ФИО владельца авто, а не водителя (машина может быть по доверенности). Только поля авто.
- Поле «Кузов» / «номер кузова» на СТС — это тот же идентификатор, что и VIN: пиши его в vin.
- Для KZ ВУ / удостоверения личности full_name пиши КИРИЛЛИЦЕЙ (фамилия имя отчество),
  как в основной строке документа. Если рядом есть латиница (BEXULTANOV MIRAS) — игнорируй её.
  Обязательно включай отчество, если оно видно (например ҚҰРМАНҒАЗЫҰЛЫ).
"""
...
def normalize_extracted_fields(
    ...
    *,
    ...
    # Owner identity on STS must not become the driver's profile.
    if document_type == "vehicle_registration_doc":
        for identity_key in (
            "full_name",
            "last_name",
            "first_name",
            ...
            fields.pop(identity_key, None)
            recognized.pop(identity_key, None)
