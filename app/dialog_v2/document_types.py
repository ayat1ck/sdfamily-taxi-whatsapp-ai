...
class DocumentTypeResolver:
    def resolve(
        ...
        *,
        ...
        if self._looks_like_selfie(text, mime, extracted_fields, current_flow):
    ...
    def _looks_like_selfie(self, text: str, mime: str, extracted_fields: dict[str, str], current_flow: str | None) -> bool:
        if "селфи" in text or "selfie" in text:
            return True
        if current_flow == "registration_document_collection" and not extracted_fields and mime.startswith("image/"):
            return True
        return False
